...
async function refresh() {
  ...
  // Stored token but getSession() didn't load it (observed: a valid, unexpired
  // session sits in storage yet getSession returns null with no error). Read the
  // tokens straight from storage and install them explicitly via setSession,
  // which bypasses the broken auto-load. Fall back to refreshSession.
  if (!session) {
    try {
