...
async function refresh() {
  ...
  state.lastError = res.error ? (res.error.message || String(res.error)) : "";
  // Safety net. normalizeStoredSession() should have repaired the storage shape
  // so getSession() works, but if some other shape slips through, install the
  // tokens explicitly via setSession (which decodes the JWT for expiry/user),
  // then fall back to refreshSession.
  if (!session) {
    try {
      const tk = Object.keys(localStorage).find((k) => k.startsWith("sb-") && k.endsWith("auth-token"));
      const o = tk ? JSON.parse(localStorage.getItem(tk) || "null") : null;
      ...
      if (s && s.access_token && s.refresh_token) {
        ...
        if (r.error) state.lastError = r.error.message || String(r.error);
      } else {
        const r = await c.auth.refreshSession();
        if (r.data && r.data.session) session = r.data.session;
        if (r.error) state.lastError = r.error.message || String(r.error);
      }
    } catch (e) { state.lastError = (e && e.message) || String(e); }
  }
  // Final fallback: Supabase still won't surface a session that is provably
  // valid in storage. Trust the stored token directly. Safe because the billing
  // server re-validates every token; this only keeps the UI in sync when
  // supabase-js's own session machinery misbehaves in this browser.
  if (!session) {
    try {
      const tk = Object.keys(localStorage).find((k) => k.startsWith("sb-") && k.endsWith("auth-token"));
      const o = tk ? JSON.parse(localStorage.getItem(tk) || "null") : null;
      const s = (o && o.access_token) ? o : (o && (o.currentSession || o.session));
      if (s && s.access_token && s.user && (!s.expires_at || s.expires_at * 1000 > Date.now())) {
        session = s;
        state.lastError = state.lastError || "using stored token directly";
      }
    } catch (e) {}
  }
