...
function broadcast() {
  ...
  if (state.user) {
    ...
    const name = meta.full_name || meta.name || (state.user.email || "Account").split("@")[0];
    el.textContent = String(name).trim().split(/\s+/)[0] || "Account";
    el.title = state.tier[0].toUpperCase() + state.tier.slice(1) + " plan";
  } else {
